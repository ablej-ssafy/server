...
public class EducationControllerTest extends RestDocsSupport {
    ...
    @Test
    @DisplayName("교육_빈_컬럼_추가_테스트")
    @CustomMockUser
    void 교육_빈_컬럼_추가_테스트() throws Exception {
        // * GIVEN: 이런게 주어졌을 때
        Long memberId = 1L;
        Long educationalId = 1L;

        EducationForm mockResponse = EducationForm.of(null, null, null, null, null, null, null, null, educationalId);

        when(educationService.createEmptyEducation(memberId))
                ...
                .andDo(restDocs.document(resource(
                        ResourceSnippetParameters.builder()
                                ...
                                .responseFields(response(
                                        fieldWithPath("data.name").type(JsonFieldType.NULL).description("학교 이름 (null)"),
                                        fieldWithPath("data.major").type(JsonFieldType.NULL).description("전공 (null)"),
                                        fieldWithPath("data.category").type(JsonFieldType.NULL).description("학교 분류 (null)"),
                                        fieldWithPath("data.grade").type(JsonFieldType.NULL).optional().description("학점 평균 (null)"),
                                        fieldWithPath("data.gradeType").type(JsonFieldType.NULL).description("최대 학점 유형 (null)"),
                                        fieldWithPath("data.description").type(JsonFieldType.NULL).optional().description("학부 활동 설명 (null)"),
                                        fieldWithPath("data.startAt").type(JsonFieldType.NULL).description("교육 시작일 (null)"),
                                        fieldWithPath("data.endAt").type(JsonFieldType.NULL).optional().description("교육 종료일 (null)"),
                                        fieldWithPath("data.educationId").type(JsonFieldType.NUMBER).description("교육 PK")
                                ))
                                .build()
                )));

        verify(educationService).createEmptyEducation(memberId);
    }
